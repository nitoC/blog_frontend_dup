...
const components: ComponentProps<typeof PortableText>["components"] = {
  ...
  block: {
    ...
    h2: ({ children }: BlockProps) => (
      /* UI Fix: Restored border-l-4 for the side accent bar design */
      <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight text-indigo-950 mt-10 mb-4 pb-1  pl-3 md:pl-4 leading-snug">
        {children}
      </h2>
    ),
    ...
    blockquote: ({ children }: BlockProps) => (
      /* UI Fix: Restored border-l-4 to contain the quote layout */
  ...
  marks: {
    ...
    link: ({ children, value }: BlockProps & { value?: { href?: string } }) => (
      <a
        ...
        /* Modernized: Removed multi-border classes; added slick hover underline state */
        className="text-emerald-600 font-semibold no-underline hover:underline hover:text-emerald-500 transition-colors duration-150 ease-in-out"
      >
        {children}
      </a>
    ),
